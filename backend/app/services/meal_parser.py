...
def looks_like_question(text: str) -> bool:
    ...
    if re.search(r"(^|\s)يا\s+ترى(\s|$)", norm):
        return True
    return is_calorie_question(text)


# أسئلة واضحة عن السعرات («احسب لي سعرات...»، «كام سعرة في...») — سؤال مش تسجيل أكل.
# محكمة عشان متلتقطش أوامر تسجيل زي «احسبهم/احسبلهم» (مفيش «لي ... سعر»).
_CALORIE_Q_RE = re.compile(
    r"احسب\s*ل[يى]\s.*سعر"      # «احسب لي ... سعرات»
    r"|احسبل[يى]\s.*سعر"
    r"|كام\s*سعر"                 # «كام سعرة في ...»
    r"|في[هها]?\s+كام\s*سعر"
    r"|عايز[ةه]?\s+اعرف\s+سعر"   # «عايز اعرف سعرات ...»
)


def is_calorie_question(text: str) -> bool:
    """True لو النص سؤال عن سعرات أكلة (نجاوبه)، مش طلب تسجيل."""
    return bool(_CALORIE_Q_RE.search(normalize(text or "")))
